...
type SidebarItem = "dashboard" | "manage-ads" | "manage-rooms" | "settings";
...
export default function AdminDashboard() {
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + formImages.length > 5) {
    ...
    files.forEach((file) => {
      ...
      reader.onloadend = () => {
        setFormImagePreviews((prev) => [...prev, reader.result as string]);
      };
  ...
  const openEditModal = (ad: Ad) => {
    ...
    setFormLink(ad.redirectLink);
    setFormImages([]);
    setFormImagePreviews(ad.images.map((img) => `http://localhost:5000${img}`));
    setIsCreateModalOpen(true);
  };

  // Create/Update Ad
  const handleSubmit = async () => {
    if (formImages.length === 0 && !editingAd) {
    ...
    formData.append("redirectLink", formLink);
  ...
  const sidebarItems: { id: SidebarItem; label: string; icon: React.ElementType }[] = [
    ...
    { id: "manage-rooms", label: "Manage Rooms", icon: DoorOpen },
        ...
        <div className="p-8">
          ...
          {activeTab === "manage-rooms" && (
            <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
              ...
              <p className="text-sm text-gray-400 mt-2">Coming soon — manage rooms, pricing, and availability</p>
            </div>
          )}
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Upload Images{" "}
                    <span className="text-gray-400 font-normal">({formImagePreviews.length}/5)</span>
                  </label>
                  <div
                  ...
                  >
                    {formImagePreviews.length > 0 ? (
                      <div className="flex flex-wrap gap-3">
                        ...
                        {formImagePreviews.length < 5 && (
                          <div className="w-24 h-24 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center text-gray-400 hover:border-[#3a7d5a] hover:text-[#3a7d5a] transition-colors">
